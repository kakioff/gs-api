...
def delete_file(cos_path: str):
    ...
    Returns:
        dict: cos返回的原始结果
    """
    res = client.delete_object(Bucket=settings.cos_bucket, Key=cos_path)
    return res

def file_exists(cos_path: str):
    """判断文件是否存在
    ...
    """
    try:
        return client.head_object(Bucket=settings.cos_bucket, Key=cos_path)
...
def get_file(cos_path: str, file_path: str | None = None):
    ...
    if file_path:
        ...
        fpb: bytes = fp.read()
        return fpb
def get_file_stream(cos_path: str):
    """获取文件流
    ...
    Returns:
        bytes: 文件流
    """
    res = client.get_object(Bucket=settings.cos_bucket, Key=cos_path)
    fp = res["Body"].get_raw_stream()
    for chunk in fp:
        yield chunk
    fp.close()
