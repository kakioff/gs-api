...
def get_all_users(
    ...
    all_count = db.exec(
        select(func.count()).select_from(Users).where(Users.role_id < user.role_name)
    ).one()

    all_users = db.exec(
        select(Users)
        .where(Users.role_id < user.role_name)
        .offset(size * (page - 1))
        .limit(size)
    ).all()
...
def update_user(
    ...
    """更新用户信息"""
    if opts.role_id is not None and opts.role_id >= usr.user.role_name:
        # 不能修改权限大于等于自己的用户权限
        return resp_err(detail="权限不足", code=403)
    user = db.exec(
        select(Users).where(Users.id == user_id, Users.role_id < usr.user.role_name)
    ).one_or_none()
    if user is None:
